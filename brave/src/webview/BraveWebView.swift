...
class BraveWebView : WKWebView {
    ...
    class func generateUniqueUserAgent() -> Int {
        struct StaticCounter {
            static var counter = 0
        }

        StaticCounter.counter += 1
        let userAgentBase = BraveWebView.webviewBuiltinUserAgent
    ...
    func stringByEvaluatingJavaScriptFromString(str: String) -> String? {
        return nil
    }

    weak var navigationDelegateWrapper: WKCompatNavigationDelegate?
    ...
    class NavDelegate : NSObject, WKNavigationDelegate {
        weak var braveWebView: BraveWebView!
        ...
        @objc func webView(webView: WKWebView, decidePolicyForNavigationAction navigationAction: WKNavigationAction, decisionHandler: (WKNavigationActionPolicy) -> Void) {
            ...
            //webView.broadcastToPageStateDelegates()
//            if let nd = webView.navigationDelegateWrapper {
//                var shouldLoad = true
...
//            }
        ...
        @objc func webView(webView: WKWebView, didStartProvisionalNavigation navigation: WKNavigation!) {
            braveWebView.broadcastToPageStateDelegates()
            braveWebView.navigationDelegateWrapper?.webViewDidStartProvisionalNavigation(braveWebView, url: webView.URL)
        }

        @objc func webView(webView: WKWebView, didCommitNavigation navigation: WKNavigation!) {
            braveWebView.broadcastToPageStateDelegates()
           // braveWebView.navigationDelegateWrapper?.we(braveWebView, didCommitNavigation: navigation)
        }

        @objc func webView(webView: WKWebView, didFinishNavigation navigation: WKNavigation!) {
            braveWebView.broadcastToPageStateDelegates()
            braveWebView.navigationDelegateWrapper?.webViewDidFinishNavigation(braveWebView, url: webView.URL)
        }
        ...
        @objc func webView(webView: WKWebView, didFailProvisionalNavigation navigation: WKNavigation!, withError error: NSError) {
            braveWebView.broadcastToPageStateDelegates()
            braveWebView.navigationDelegateWrapper?.webViewDidFailNavigation(braveWebView, withError: error)
        }
    }
    ...
    override func observeValueForKeyPath(keyPath: String?, ofObject object: AnyObject?, change: [String: AnyObject]?, context: UnsafeMutablePointer<Void>) {
        ...
        case KVOLoading:
            ...
            delegatesForPageState.forEach { $0.value?.webView(self, isLoading: loading) }
...
//            }
        ...
        case KVOCanGoBack:
            ...
            delegatesForPageState.forEach { $0.value?.webView(self, canGoBack: canGoBack) }

        case KVOCanGoForward:
            guard let canGoForward = change?[NSKeyValueChangeNewKey] as? Bool else { break }

            delegatesForPageState.forEach { $0.value?.webView(self, canGoForward: canGoForward) }
      default:
