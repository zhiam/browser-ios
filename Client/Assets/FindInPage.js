...
function highlightAllMatches(text) {
  ...
  if (!text.trim()) {
    __bravejs___findInPageHandler({ totalResults: 0 });
    return;
  }
  ...
  __bravejs___findInPageHandler({ totalResults: matches.length });
}
...
function updateSearch(text) {
  ...
  __bravejs___findInPageHandler({ currentResult: currentResult });
