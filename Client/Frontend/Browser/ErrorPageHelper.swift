...
class ErrorPageHelper {
        ...
        server.registerHandlerForMethod("GET", module: "errors", resource: "error.html", handler: { (request) -> GCDWebServerResponse! in
            ...
            if errDomain == kCFErrorDomainCFNetwork as String {
                ...
                variables["warning_actions"] =
                    "<p><a href='javascript:__bravejs___errorPageHelperMessageManager({type: \"\(MessageCertVisitOnce)\"})'>\(Strings.ErrorPagesVisitOnceButton)</button></p>"
            }
    ...
    func showPage(error: NSError, forUrl url: NSURL, inWebView webView: BraveWebView) {
