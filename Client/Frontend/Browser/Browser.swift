...
let session = sessionData
    ...
    var deletedWebViewHtml: (url: String, html: String)?
    func deleteWebView() {
        assert(NSThread.isMainThread())
        if !NSThread.isMainThread() {
            return
        ...
        if let webView = webView {
            ...
            if currentItem != nil {
                ...
                if let url = webView.URL?.absoluteString, html = webView.stringByEvaluatingJavaScriptFromString("document.documentElement.outerHTML") {
                    if let existingUrl = deletedWebViewHtml?.url where url == existingUrl {}
                    else {
                        deletedWebViewHtml = (url, html)
                    }
                }
            }
